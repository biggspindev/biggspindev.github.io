...
function navtospecial(destination, id) {
  if (id == 0) {
    ...
    localStorage.setItem("istitleshort", true);
    localStorage.setItem("desc", "A project has 4 possible states:<br>-Date: If the satus is a date, the project will release / be finished at that date.<br>-InDev: The project is currently being worked on, and there is no release date yet.<br>-On Hold: This project isnt cancelled, but the is currently no work done on ths project.<br>-Cancelled, this project will forever be unfinished.");
    localStorage.setItem("buttons", false);
    localStorage.setItem("buttonsdestin", false);
  } else if (id == 1) {
    localStorage.setItem("title", "About Us");
    localStorage.setItem("istitleshort", true);
    localStorage.setItem("desc", "We are an aspiring team of developpers who have interest in making stuff on the internet, if you are interested in teaming up, contact one of our team members on discord.");
    localStorage.setItem("buttons", false);
    localStorage.setItem("buttonsdestin", false);
  } else if (id == 2) {
